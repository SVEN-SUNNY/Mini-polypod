...
DC_BASE_SPEED = 25             # Reduced base speed (25%) for DC motors
...
def encoder_polling_thread():
    """Thread that continuously polls encoder states"""
    ...
    while encoder_polling_active:
        try:
            ...
            # Define state transitions (Gray code sequence)
            transitions = {
                (0, 1): -1,  # 00 -> 01: CCW
                (0, 2): 1,   # 00 -> 10: CW
                (1, 3): 1,   # 01 -> 11: CW
            ...
            if direction1:
                encoder1_count -= direction1  # Inverted for physical mounting
                encoder1_last_state = new_state1
            
            # Update encoder 2
            direction2 = transitions.get((encoder2_last_state, new_state2), 0)
            if direction2:
                encoder2_count -= direction2  # Inverted for consistency
